...
const PlayedGamesScreen: FC = () => {
  ...
  const loadMoreGames = () => {
    ...
    GameService.getOwnGames(user!.id, 5, games[games.length - 1].id).then(games => {
      setGames(current => [
        ...current,
        ...games.filter((game: Game) => !current.some(g => game.id === g.id)),
      ]);
              ...
              >
                <TouchableRipple onPress={goToGame(game)}>
                  <View style={styles.listItem}>
                    {game.finished ? (
                      <Icon style={styles.listItemIcon} color="white" name="check-bold" />
                    ) : (
                      <Icon style={styles.listItemIcon} color="white" name="pause" />
                    )}
                    <View>
                      ...
                      <Text style={styles.listItemGameTimes}>
                        Started at {format(game.started, 'LLL dd, yyyy - HH:mm')}
                      </Text>
                    </View>
                  </View>
                </TouchableRipple>
