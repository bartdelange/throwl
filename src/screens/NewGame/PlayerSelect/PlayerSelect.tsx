...
import { PlayerItem } from '~/screens/NewGame/PlayerSelect/components/PlayerItem/PlayerItem';
import { useAppTheme } from '~/App/theming.tsx';
import { GameService } from '~/services/game_service.ts';
import { FormInput } from '~/components/FormInput/FormInput.tsx';
import { useStyles } from '~/screens/NewGame/PlayerSelect/PlayerSelect.styles.ts';
...
export const PlayerSelectScreen = () => {
  ...
  const [selectedUsers, _setSelectedUsers] = useState<string[]>(route.params?.selectedUsers || []);
  const [guestUsers, setGuestUsers] = useState<string[]>(route.params?.guestUsers || []);
          ...
          <FlatList<Omit<User, 'friends'> | GuestUser>
            ...
            ListEmptyComponent={
              <Text style={styles.header}>
                You have not added any friends yet, you can also add a guest using the menu above
              </Text>
            }
                    ...
                    onPress={() =>
                      setSelectedUsers(current => {
                        if (current.indexOf(subbedPlayer.id) >= 0) {
                          return current.filter(userId => userId !== subbedPlayer.id);
                        } else {
      ...
      <AppModal
        ...
        actions={
          <View>
            ...
            />
            <LogoButton mode="light" label="ADD" onPress={addGuest} style={styles.button} />
          </View>
        }
      />
    </FullScreenLayout>
