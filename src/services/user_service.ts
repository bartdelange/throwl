...
export class UserService extends FirebaseService {
  ...
  public static async create(uid: string, email: string, name: string): Promise<User> {
    const usersCollection = this.getCollection('users');
  ...
  public static async removeFriend(uid: string, fid: string, requesterId?: string) {
    const usersCollection = this.getCollection('users');
  ...
  private static async parseUser(uid: string, user: UserDoc | undefined): Promise<User> {
    if (!user) {
