...
import { GameStatsHelper } from '~/lib/game_stats_helper.ts';

export type TurnNeeded = [Throw | undefined, Throw | undefined, Throw | undefined];
...
export class GameHelper {
  ...
  static calculateScore(turns: Turn[], checkValid: boolean = false): number {
    return (turns ?? [])
  ...
  static getNextUserIndex(args: {
    ...
    const { activeUserIndex, playersLength, reverse = false, customIndex } = args;
  ...
  static undoLastThrow(args: { turns: Turn[]; currentTurn: Turn }): {
    ...
    let turnCopy: Turn = { ...args.currentTurn, throws: [...(args.currentTurn.throws ?? [])] };
    ...
    if (!(turnCopy.throws?.length ?? 0)) {
      ...
      if (!lastTurn) {
        return { turns: args.turns, currentTurn: args.currentTurn, didPopTurn: false };
      }
