...
export class ScoreHelper {
        ...
        for (const thrw of throws) {
            if (throwCountMap.has(`${thrw.type}/${thrw.score}`)) {
                throwCountMap.set(`${thrw.type}/${thrw.score}`, {
                    throw: thrw,
                    count:
                        (throwCountMap.get(`${thrw.type}/${thrw.score}`)
                            ?.count || 1) + 1,
                });
            } else {
                throwCountMap.set(`${thrw.type}/${thrw.score}`, {
        ...
        if (throwCount.length && min !== null && max !== null) {
            for (const throwCountElement of throwCount) {
                generatedHeatmap.set(
                    ...
                    max - min === 0
                        ? 1
                        : (throwCountElement[1].count - min) / (max - min)
                );
            }
        }
    ...
    static getThrowStats(turns: Turn[]): ThrowStats {
        ...
        const out = throws.reduce<number>(
            (previousValue, element) =>
                previousValue +
                (element.type == DartboardScoreType.Out ? 1 : 0),
            0
        );
    ...
    static calculateScore(
        ...
        return userTurns
            ...
            .reduce<number>(
                (prev, turn) => prev + this.calculateTurnScore(turn),
                0
            );
    }
    ...
    static calculateScoreStatsForUser(
        ...
        return {
            ...
            validAvg: Number(
                (
                    summedValidScore /
                        userTurns.filter(t => t.isValid).length || 0
                ).toFixed(2)
            ),
            score: startingScore - (summedValidScore + currentTurnScore),
            turns: userTurns.length,
            throws:
                userTurns.flatMap(turn => turn.throws).length +
