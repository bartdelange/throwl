...
import { DoublesGameHelper } from '@throwl/shared-domain-game-rules';
import {
  MaterialDesignIcons,
...
export const GraphContainer: FC<ScoreContainerProps> = ({
  turns,
  gameOptions,
}) => {
  ...
  const targetsInGame = useMemo(
    () => DoublesGameHelper.buildTargets(gameOptions),
    [gameOptions],
  );

  const makeIconLabel = useCallback(
    (name: MaterialDesignIconsIconName, color: string) => () => (
      <MaterialDesignIcons
        name={name}
        size={14}
        color={color}
        style={styles.topLabelIcon}
      />
    ),
    [styles.topLabelIcon],
  );
