...
import 'package:dartapp/components/dartboard_icon_button.dart';
...
class NewGameState extends State<NewGameScreen>
  ...
  List<user_models.User> shuffle(List<user_models.User> items,
      math.Random random) {
                          ...
                          children: [
                            ...
                            Expanded(
                              child: ScrollableListView(
                                children: users.map((friend) =>_renderFriendItem(user.userId, friend)).toList(),
                              ),
                            ),
                            _renderStartButton(user),
                          ],
                        ),
                      ),
                    ),
  ...
  Widget _renderFriendItem(String forcedId, user_models.User friend) {
    return GestureDetector(
      onTap: () {
        if (forcedId == friend.userId) return;
                ...
                child: Container(
                  ...
                  child: Checkbox(
                    ...
                    activeColor: Theme
                        .of(context)
                        .primaryColor,
                    value: forcedId == friend.userId || _selectedUsers.any(
                          (selectedUser) =>
                      selectedUser.userId == friend.userId,
                    ),
                    onChanged: forcedId == friend.userId ? null : (bool? value) {
                      setState(() {
                        if (value!) {
                          _selectedUsers.add(friend);
                        } else {
  ...
  Widget _renderStartButton(user_models.User currentUser) {
    return Padding(
      padding: EdgeInsets.only(
        top: 20.h,
        right: 20.w,
